...
func add_skill(init_id: String, init_name: String):
	if DICTIONARY.has(init_id):
		print("DUPLICATE SKILL: ", init_id, " ", init_name)
	else:
		DICTIONARY[init_id] = Skill.new(init_id, init_name)
